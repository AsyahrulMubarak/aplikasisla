...
function calculate(events, options = {}) {
  const { branch = 'Kendari', role = 'teknisi', now = '2026-09-09T08:00:00+08:00', month = '2026-09', luarKota = '', liburTambahan = '' } = options;
...
test('Pokok x2 hanya menggandakan upah pokok harian, bukan lembur', () => {
  ...
  assert.ok(Math.abs(duaKali.totalUpahHadir - biasa.totalUpahHadir * 2) < 0.000001);
  assert.ok(Math.abs(duaKali.totalLembur - biasa.totalLembur) < 0.000001);
  assert.ok(Math.abs((duaKali.totalBersih - biasa.totalBersih) -
    biasa.totalUpahHadir) < 0.000001);
  assert.match(duaKali.barisHTML, /Upah pokok harian x2 \(Luar Kota\)/);
});

test('Libur tambahan manual menggandakan pokok harian, libur nasional tidak otomatis', () => {
  const events = [event('08:00:00', 'Masuk'), event('18:00:00', 'Keluar')];
  const biasa = calculate(events);
  const libur = calculate(events, { liburTambahan: '8' });
  assert.ok(Math.abs(libur.totalUpahHadir - biasa.totalUpahHadir * 2) < 0.000001);
  assert.ok(Math.abs(libur.totalLembur - biasa.totalLembur) < 0.000001);
  assert.match(libur.barisHTML, /Upah pokok harian x2 \(Libur Tambahan\)/);
  const nasional = calculate([
    event('08:00:00', 'Masuk', '2026-08-17'), event('18:00:00', 'Keluar', '2026-08-17')
  ], { month: '2026-08', now: '2026-08-18T08:00:00+08:00' });
  assert.doesNotMatch(nasional.barisHTML, /Hari Libur Nasional|Libur Tambahan/);
});
