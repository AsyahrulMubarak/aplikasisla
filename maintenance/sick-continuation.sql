...
returns void language sql security definer set search_path = pg_catalog, public as $$
  ...
  set kembali_bekerja_pada = public.sla_waktu_kembali_pengajuan(p.nama_pegawai, p.tanggal_mulai, case when p.jenis = 'Sakit' then null else p.tanggal_selesai end)
  where p.nama_pegawai = p_nama and p.status = 'Disetujui'
    and p.kembali_bekerja_pada is distinct from public.sla_waktu_kembali_pengajuan(p.nama_pegawai, p.tanggal_mulai, case when p.jenis = 'Sakit' then null else p.tanggal_selesai end);
$$;
...
begin
  ...
  NEW.kembali_bekerja_pada := case when NEW.status = 'Disetujui'
    then public.sla_waktu_kembali_pengajuan(NEW.nama_pegawai, NEW.tanggal_mulai, case when NEW.jenis = 'Sakit' then null else NEW.tanggal_selesai end)
    else null end;
  return NEW;
end;
$$;
...
before insert or update of status, nama_pegawai, jenis, tanggal_mulai, tanggal_selesai
